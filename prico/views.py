		...
		if check_length([[email,100],[first_name,100],[last_name,100],[password,100]]) and check_blank_or_null([email,first_name,last_name,password]) :
			if User.objects.filter(username=email).exists():
				...
				if password.isalnum() == False:
					...
					context['status']="Password must be strong .\nso no one can able to see you orders and other information.\nPassword must be alpha numeric"	
		else:
...
def index(request):
	...
	context['best_category']=best_category.objects.all().order_by("id")
	offer=[]
...
def select_state(request):
	context={}
	if request.method=="POST":
		pk=request.POST.get("state_id")
		if check_blank_or_null([pk]) and pk.isdigit() and state.objects.filter(pk=pk).exists():
			con=state.objects.get(pk=pk)
			context['city']=list(city.objects.filter(state=con).values("id","name"))
		return JsonResponse(context)
...
def select_city(request):
	context={}
	if request.method=="POST":
		pk=request.POST.get("city_id")
		if check_blank_or_null([pk]) and pk.isdigit() and city.objects.filter(pk=pk).exists():
			con=city.objects.get(pk=pk)
			context['area']=list(area.objects.filter(city=con).values("id","name"))
			print(context)
		return JsonResponse(context)
...
def checkout(request):
	...
	if request.method=="POST":
		...
		try:
			...
			prc=""
		prc_require = False 
		if check_blank_or_null([zipcode,address,mobile,userid,state_id,city_id,area_id]) and check_length([[zipcode,50],[address,400],[mobile,22]]) and state_id.isdigit() and city_id.isdigit() and area_id.isdigit() :
			if state.objects.filter(pk=state_id).exists() and city.objects.filter(pk=city_id).exists() and area.objects.filter(pk=area_id).exists():
				if check_cookies(userid):
					cust=custid.objects.get(uid=userid)
					if cart.objects.filter(custid=cust).count() > 0:
						total_price=0
						...
						o.mobile=mobile
						o.notes=notes
						o.state=state.objects.get(pk=state_id).name
						o.city=city.objects.get(pk=city_id).name
						o.area=area.objects.get(pk=area_id).name
				...
				context['status']="Area Is not exists.Sorry we can not able reach the place.May we can reach this place by super car"		
		else:
	...
	if request.method=="POST":
		...
		if check_cookies(userid):
			...
			if cart.objects.filter(pk=pk,custid=cust).exists():
				...
				c.quantity+=1
				c.save()
				context['qty']=c.quantity
				context['status']="Cart has been updated and be healthy".title()
			else:
	...
	if request.method=="POST":
		...
		if check_cookies(userid):
			...
			if cart.objects.filter(pk=pk,custid=cust).exists():
				c=cart.objects.get(pk=pk,custid=cust)
				if c.quantity == 1:
					context['status']="Can not decrease the cart"
				else:	
					c.quantity-=1
					c.save()
					context['qty']=c.quantity
...
def update_remember_list(request):
	...
	if request.method=="POST":
		pk=request.POST.get("remid")
		userid=request.POST.get("userid")
		if check_blank_or_null([userid,pk]) and check_cookies(userid):
			cust=custid.objects.get(uid=userid)
			if remember.objects.filter(pk=pk,custid=cust).exists():
				c=remember.objects.get(pk=pk,custid=cust)
				c.quantity+=1
				c.save()
				context['qty']=c.quantity
				context['status']="Remember List has been updated and be healthy".title()
			else:
				context['status']='Data Is not right Format and make yourself fit for everythink'.title()
	else:
		context['status']="fail"
	return JsonResponse(context)
...
@csrf_exempt
def minus_update_remember_list(request):
	context={}
	if request.method=="POST":
		pk=request.POST.get("remid")
		userid=request.POST.get("userid")
